...
class InternalLogger {
    static String getCurrentTimestamp(){
        return "["+String.format("%1\$tH:%1\$tM:%1\$tS", Calendar.instance)+"]"
    }
    
    static void log(msg) {
        //println "====== Debug =======${getCurrentTimestamp()}[${Thread.currentThread()}]: ${msg}"
    }
}
				...
				thread = Thread.startDaemon(
						{
							if (slow == null) {
                                ...
                                InternalLogger.log "Sending finished..."
								outBytes = null
								outLen = 0
							} else {
								int ptr = outPtr
...
class InputSession implements java.io.Closeable {
	...
	String expect(Object expectations, long timeout) {
		...
		synchronized (lock) {
			...
			receivedObject = null
			receivedUntil = null
			InternalLogger.log "Start expection of ${expectations}..."
		...
		synchronized (lock) {
			...
			if (expectations != null) {
				...
				if (pos != null) {                    
					...
					expectations = null
                    InternalLogger.log "Found!!! caret: ${caret}, pos: ${pos}, stringBuffer length: ${ stringBuffer.length()}"
					lock.notify()
					return true
				}
			}
